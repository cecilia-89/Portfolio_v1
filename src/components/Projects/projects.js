...
const Projects = () => {
    ...
    useEffect(() => {
        ...
        if (swipeRight) {
            gsap.fromTo(projects.current, {opacity: 0 , x: '100vw'}, {opacity: 1, x:'0',  display: 'block', duration: 1})
        } else {
            gsap.fromTo(projects.current, {opacity: 1 , x: '0'}, {opacity: 0, x:'100vw', display: 'none', duration: 1})
        }
